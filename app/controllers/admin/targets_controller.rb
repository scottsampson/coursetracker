class Admin::TargetsController < Admin::ApplicationController
  # GET /targets
  # GET /targets.xml
  def index
    @targets = Target.all
  ...
  def create
    ...
    respond_to do |format|
      if @target.save
        format.html { redirect_to([:admin,@target], :notice => 'Target was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @target.update_attributes(params[:target])
        format.html { redirect_to([:admin,@target], :notice => 'Target was successfully updated.') }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(admin_targets_url) }
      format.xml  { head :ok }
    end
  end
end
