class Admin::ResourcesController < Admin::ApplicationController
  ...
  def index
    @admin_resources = Resource.find_all_by_course_id(params[:course_id])
    
    @admin_resources = [] unless !@admin_resources.nil?
    @course_id = params[:course_id]
    puts @admin_resources.inspect
    
    respond_to do |format|
      format.html # index.html.erb
      format.xml  { render :xml => @admin_resources }
    end
  end
