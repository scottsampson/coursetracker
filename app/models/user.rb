class User < ActiveRecord::Base
  ...
  def known_courses
    self.courses.collect{|course| course if course.courses_users.first.know > 0 }.compact
  end
  
  def known_beginner
    self.courses.collect{|course| course if course.courses_users.first.know > 0 && course.level_id == 1 }.compact
  end
  
  def known_intermediate
    self.courses.collect{|course| course if course.courses_users.first.know > 0 && course.level_id == 2 }.compact
  end
  
  def known_expert
    self.courses.collect{|course| course if course.courses_users.first.know > 0 && course.level_id == 3 }.compact
  end
